...
/**
 * Tes validasi backend untuk endpoint form kesediaan dari project-akhir.
 * Memastikan input sembarangan ditolak (422) sebelum tersimpan ke database.
 *
 * Jalankan:  php artisan test --filter=FormKesediaanValidationTest
 */
class FormKesediaanValidationTest extends TestCase
{
    private string $endpoint = '/api/form-kesediaan/store-from-external';
    ...
    {
        $this->putJson('/api/form-kesediaan/1/status', ['status' => 'ngawur'])
            ->assertStatus(422)
